...
fn test_buy(){
    ...
    let info = mock_info("user1", &[Coin{denom:"ujuno".to_string(), amount:Uint128::new(10)}]);
    let msg = ExecuteMsg::BuyToken {  };
    execute(deps.as_mut(), env.clone(), info, msg).unwrap();
    
    let info = mock_info("user1", &[Coin{denom:"ujuno".to_string(), amount:Uint128::new(15)}]);
    let msg = ExecuteMsg::BuyToken {  };
    let res = execute(deps.as_mut(), env.clone(), info, msg).unwrap();
    assert_eq!(res.messages.len(), 1);
    assert_eq!(res.messages[0].msg, CosmosMsg::Bank(BankMsg::Send{ 
        to_address: "admin".to_string(), 
        amount: vec![Coin{denom:"ujuno".to_string(), amount: Uint128::new(15)}]}
    ));

    let info = mock_info("user2", &[Coin{denom:"ujuno".to_string(), amount:Uint128::new(20)}]);
    let msg = ExecuteMsg::BuyToken {  };
    execute(deps.as_mut(), env.clone(), info, msg).unwrap();    

    let info = mock_info("user1", &[Coin{denom: ATOM.to_string(), amount:Uint128::new(10)}]);
    let msg = ExecuteMsg::BuyToken {  };
    execute(deps.as_mut(), env.clone(), info, msg).unwrap();
    ...
    let info = mock_info("user2", &[Coin{denom: ATOM.to_string(), amount:Uint128::new(20)}]);
    let msg = ExecuteMsg::BuyToken {  };
    let res = execute(deps.as_mut(), env.clone(), info, msg).unwrap();
    assert_eq!(res.messages.len(), 1);
    assert_eq!(res.messages[0].msg, CosmosMsg::Bank(BankMsg::Send{
...
fn claim() {
    ...
    execute(deps.as_mut(), env.clone(), info, msg).unwrap();
     ...
     let info = mock_info("user2", &[Coin{denom:"ujuno".to_string(), amount:Uint128::new(10)}]);
    let msg = ExecuteMsg::BuyToken {  };
    execute(deps.as_mut(), env.clone(), info, msg).unwrap();

    env.block.time = env.block.time.plus_seconds(250);

    let claimable_amount = query_claimable_amount(deps.as_ref(), env.clone(), "user1".to_string()).unwrap();
    ...
    let claimable_time = query_claimable_time(deps.as_ref(), env.clone(), "user1".to_string()).unwrap();
    println!("{:?}", claimable_time.claimable_time - claimable_time.crr_time);

    assert_eq!(res.messages.len(), 1);
    assert_eq!(res.messages[0].msg, CosmosMsg::Wasm(WasmMsg::Execute { 
        contract_addr: "token_address".to_string(), 
        msg: to_binary(&Cw20ExecuteMsg::Transfer{ recipient: "user1".to_string(), amount: Uint128::new(125)}).unwrap(), 
        funds: vec![] }
    ));
    ...
    assert_eq!(res.messages[0].msg, CosmosMsg::Wasm(WasmMsg::Execute { 
        contract_addr: "token_address".to_string(), 
        msg: to_binary(&Cw20ExecuteMsg::Transfer{ recipient: "user1".to_string(), amount: Uint128::new(75)}).unwrap(), 
        funds: vec![] }
    ));
    ...
    assert_eq!(res.messages[0].msg, CosmosMsg::Wasm(WasmMsg::Execute { 
        contract_addr: "token_address".to_string(), 
        msg: to_binary(&Cw20ExecuteMsg::Transfer{ recipient: "user1".to_string(), amount: Uint128::new(150)}).unwrap(), 
        funds: vec![] }
    ));
    ...
    assert_eq!(res.messages[0].msg, CosmosMsg::Wasm(WasmMsg::Execute { 
        contract_addr: "token_address".to_string(), 
        msg: to_binary(&Cw20ExecuteMsg::Transfer{ recipient: "user2".to_string(), amount: Uint128::new(500)}).unwrap(), 
        funds: vec![] }
    ));
