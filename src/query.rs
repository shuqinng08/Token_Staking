use crate::msg::{
    QueryMsg, UserInfoResponse, UserInfosResponse, TimeInfo
};
use crate::state::{
     State, CONFIG, UserInfo,  SaleInfo,  SALEINFO, user_info_storage, user_info_key
};
use cosmwasm_std::{entry_point, to_binary, Binary, Deps, Env,  StdResult, Uint128, Decimal, Order};
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::GetUserInfos { start_after, limit } => to_binary(&query_get_user_infos(deps,start_after,limit)?)
    }
}
